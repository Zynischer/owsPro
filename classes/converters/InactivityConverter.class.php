...
class InactivityConverter implements IConverter {
	...
	public function toHtml($row) {
		...
		$output = '<a href=\'#actPopup'. $row['id']. '\' role=\'button\' data-toggle=\'modal\' title=\''.getMessage('manage_show_details') . '\' style=\'color: '. $color .'\'>'.$rate .' %</a>';
	...
	private function _renderInActivityPopup($row) {
		...
		$popup .= '<div class=\'modal-header\'><button type=\'button\' class=\'close\' data-dismiss=\'modal\' aria-hidden=\'true\' title=\''.getMessage('button_close') . '\'>&times;</button>';
		$popup .= '<h3 id=\'actPopupLabel'. $row['id']. '\'>'.getMessage('entity_user_inactivity') . ': '. escapeOutput($row['entity_users_nick']) . '</h3></div>';
		...
		$popup .= '<table class=\'table table-bordered\'>
          <thead><tr>
            <th>'.getMessage('popup_user_inactivity_title_action') . '</th>
            <th>'.getMessage('entity_user_inactivity') . '</th>
          </tr></thead>
          <tbody><tr>
            <td><b>'.getMessage('entity_user_inactivity_login') . '</b><br>
            <small>'.getMessage('entity_users_lastonline') . ': '. date('d.m.y, H:i',$row['entity_users_lastonline']) .'</small></td>
          ...
          <tr>
            <td><b>'.getMessage('entity_user_inactivity_aufstellung') . '</b></td>
          ...
          <tr>
            <td><b>'.getMessage('entity_user_inactivity_transfer') . '</b><br>
            <small>'. sprintf(getMessage('entity_user_inactivity_transfer_check'), date('d.m.y, H:i',$row['entity_user_inactivity_transfer_check'])) . '</small></td>
          ...
          <tr>
            <td><b>'.getMessage('entity_user_inactivity_vertragsauslauf') . '</b></td>
          ...
          <tfoot>
          <tr>
            <td><b>'.getMessage('popup_user_inactivity_total') . '</b></td>
		...
		$popup .= '</div>';
		$popup .= '<div class=\'modal-footer\'><button class=\'btn btn-primary\' data-dismiss=\'modal\' aria-hidden=\'true\'>'.getMessage('button_close') . '</button></div>';
		$popup .= '</div>';
		return $popup;
	}

...
class SimulationAudienceCalculator {
	...
	public static function computeAndSaveAudience(WebSoccer $websoccer, DbConnection $db, SimulationMatch $match) {
		...
		$maintenanceInfluence = $homeInfo['level_videowall'] * getConfig('stadium_videowall_effect');
		$maintenanceInfluenceSeats = (5 - $homeInfo['level_seatsquality']) * getConfig('stadium_seatsquality_effect');
		$maintenanceInfluenceVip = (5 - $homeInfo['level_vipquality']) * getConfig('stadium_vipquality_effect');
		...
		// call plug-ins
		$event = new TicketsComputedEvent($websoccer, $db, I18n::getInstance(getConfig('supported_languages')),
	...
	private static function updateMaintenanceStatus(WebSoccer $websoccer, DbConnection $db, $homeInfo) {
		...
		foreach ($types as $type) {
			if ($columns['maintenance_' . $type] <= 0) {
				$columns['maintenance_' . $type] = getConfig('stadium_maintenanceinterval_' . $type);
	...
	private static function weakenPlayersDueToGrassQuality(WebSoccer $websoccer, $homeInfo, SimulationMatch $match) {
		...
		$strengthChange = (5 - $homeInfo['level_pitch']) * getConfig('stadium_pitch_effect');

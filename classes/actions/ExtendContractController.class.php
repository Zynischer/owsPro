...
class ExtendContractController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($satisfaction < MINIMUM_SATISFACTION_FOR_EXTENSION) {
			throw new Exception(getMessage("extend-contract_player_is_unhappy"));
		}

		// check if player is already on market
		if ($player["player_transfermarket"]) {
			throw new Exception(getMessage("sell_player_already_on_list"));
		}

		// no salary decrease
		if ($parameters["salary"] < $player["player_contract_salary"]) {
			throw new Exception(getMessage("extend-contract_lower_than_current_salary"));
		}
		...
		if ($parameters["salary"] < $minSalary) {
			...
			throw new Exception(getMessage("extend-contract_salary_too_low"));
		}
		...
		if ($parameters["goal_bonus"] < $minGoalBonus) {
			throw new Exception(getMessage("extend-contract_goalbonus_too_low"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("extend-contract_success"),""));

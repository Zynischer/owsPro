...
class UploadClubPictureController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if (!getConfig("upload_clublogo_max_size")) {
		...
		if (!in_array($ext, $allowedExtensions)) {
			throw new Exception(getMessage("change-profile-picture_err_illegalfiletype"));
		}
		...
		$maxFilesize = getConfig("upload_clublogo_max_size") * 1024;
		...
		if ($errorcode == UPLOAD_ERR_OK) {
			...
			if (!$uploaded) {
				throw new Exception(getMessage("change-profile-picture_err_failed"));
			}
		} else {
			throw new Exception(getMessage("change-profile-picture_err_failed"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("upload-clublogo_success"),""));

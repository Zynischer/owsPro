...
class BorrowPlayerController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($clubId == $player["team_id"]) {
			throw new Exception(getMessage("lending_hire_err_ownplayer"));
		}

		// check if player is borrowed by any user
		if ($player["lending_owner_id"] > 0) {
			throw new Exception(getMessage("lending_hire_err_borrowed_player"));
		}

		// check if player is offered for lending
		if ($player["lending_fee"] == 0) {
			throw new Exception(getMessage("lending_hire_err_notoffered"));
		}

		// check if player is on transfermarket
		if ($player["player_transfermarket"] > 0) {
			throw new Exception(getMessage("lending_err_on_transfermarket"));
		}
		...
		if ($parameters["matches"] >= $player["player_contract_matches"]) {
			throw new Exception(getMessage("lending_hire_err_contractendingtoosoon", $player["player_contract_matches"]));
		}
		...
		if ($team["team_budget"] < $minBudget) {
			throw new Exception(getMessage("lending_hire_err_budget_too_low"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("lending_hire_success"),""));

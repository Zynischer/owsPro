...
class MoveYouthPlayerToProfessionalController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($clubId != $player["team_id"]) {
			throw new Exception(getMessage("youthteam_err_notownplayer"));
		}
		...
		if (!in_array($parameters["mainposition"], $validPositions)) {
			throw new Exception(getMessage("youthteam_makeprofessional_err_invalidmainposition"));
		}
		...
		if ($team["team_budget"] <= TeamsDataService::getTotalPlayersSalariesOfTeam($this->_websoccer, $this->_db, $clubId)) {
			throw new Exception(getMessage("youthteam_makeprofessional_err_budgettooless"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("youthteam_makeprofessional_success"),""));

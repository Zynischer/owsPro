...
class DirectTransferOfferController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($parameters["exchangeplayer1"] || $parameters["exchangeplayer2"]) {
			...
			if ($teamSize < (getConfig("transfermarket_min_teamsize") - $numberOfSizeReduction)) {
		...
		if ($team["team_budget"] < $totalOffers) {
			throw new Exception(getMessage("transferoffer_err_totaloffers_too_high"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("transferoffer_submitted_title"),
				getMessage("transferoffer_submitted_message")));
	...
	private function checkIfThereAreAlreadyOpenOffersFromUser($teamId) {
		...
		if ($count["hits"]) {
			throw new Exception(getMessage("transferoffer_err_open_offers_exist"));
		}
	}
	...
	private function checkIfUserIsAllowedToSendAlternativeOffers($playerId) {
		...
		if ($count["hits"]) {
			throw new Exception(getMessage("transferoffer_err_noalternative_allowed"));
		}
	}
	...
	private function checkPlayersTransferStop($playerId) {
		...
		if (getConfig("transferoffers_transfer_stop_days") < 1) {
			return;
		}
		...
		if ($count["hits"]) {
			throw new Exception(getMessage("transferoffer_err_transferstop",getConfig("transferoffers_transfer_stop_days")));
		}
	}
	...
	private function checkExchangePlayer($playerId) {
		...
		if ($player["player_transfermarket"] || $player["team_user_id"] != $this->_websoccer->getUser()->id) {
			throw new Exception(getMessage("transferoffer_err_exchangeplayer_on_transfermarket", $playerName));
		}
		...
		if ($count["hits"]) {
			throw new Exception(getMessage("transferoffer_err_exchangeplayer_involved_in_other_offers", $playerName));
		}
		...
		try {
			...
			throw new Exception(getMessage("transferoffer_err_exchangeplayer_transferstop", $playerName));
		}
	}

...
class MicropaymentRedirectController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		$projectId = trim(getConfig("micropayment_project"));
		if (!strlen($projectId)) {
			throw new Exception("Configuration error: micropayment.de project ID is not specified.");
		}

		$accessKey = trim(getConfig("micropayment_accesskey"));
		if (!strlen($accessKey)) {
		...
		$validModules = array();
		if (getConfig("micropayment_call2pay_enabled")) {
			$validModules[] = 'call2pay';
		}
		if (getConfig("micropayment_handypay_enabled")) {
			$validModules[] = 'handypay';
		}
		if (getConfig("micropayment_ebank2pay_enabled")) {
			$validModules[] = 'ebank2pay';
		}
		if (getConfig("micropayment_creditcard_enabled")) {
		...
		if (getConfig('premium_currency') != 'EUR') {

...
class SaveMatchChangesController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($matchinfo['match_simulated']) {
			throw new Exception(getMessage('match_details_match_completed'));
		}
		...
		if (count($occupiedSubPos) < 3) {
			...
			$nextPossibleMinute = $matchinfo['match_minutes'] + getConfig('sim_interval') + 1;
				...
				if ($slot && $newOut && $newIn && $newMinute) {
					if ($saveSub && $newMinute < $nextPossibleMinute) {
						...
						$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_WARNING,
								'',
								getMessage('match_details_changes_too_late_altered', $subNo)));
					}
		...
		if ($prevOffensive !== $parameters['offensive']
			...
			if ($alreadyChanged >= getConfig('sim_allow_offensivechanges')) {
				throw new Exception(getMessage('match_details_changes_too_often',
						getConfig('sim_allow_offensivechanges')));
			}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage('saved_message_title'),''));

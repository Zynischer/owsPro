...
class DisableAccountController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($clubId) {
			$this->_db->queryUpdate(array("user_id" => '', "captain_id" => ''),"_verein",
					"user_id = %d", $this->_websoccer->getUser()->id);
		}

		// disable user
		$this->_db->queryUpdate(array("status" => "0"),"_user",
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,getMessage("cancellation_success"),""));

...
class BuyYouthPlayerController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($clubId == $player["team_id"]) {
			throw new Exception(getMessage("youthteam_buy_err_ownplayer"));
		}

		// player must not be tranfered from one of user's other teams
		$result = $this->_db->querySelect("user_id",getConfig("db_prefix") . "_verein", "id = %d", $player["team_id"]);
		$playerteam = $result->fetch_array();
		$result->free_result();
		if ($playerteam["user_id"] == $user->id) {
			throw new Exception(getMessage("youthteam_buy_err_ownplayer_otherteam"));
		}

		// check if enough budget
		$team = TeamsDataService::getTeamSummaryById($this->_websoccer, $this->_db, $clubId);
		if ($team["team_budget"] <= $player["transfer_fee"]) {
			throw new Exception(getMessage("youthteam_buy_err_notenoughbudget"));
		}
		...
		$this->_db->queryUpdate(array("team_id" => $clubId, "transfer_fee" => 0),
				getConfig("db_prefix") . "_youthplayer", "id = %d", $parameters["id"]);
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("youthteam_buy_success"),""));

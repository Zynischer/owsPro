...
class UpgradeStadiumController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		// upgradable?
		if ($existingLevel >= 5) {
			throw new Exception(getMessage("stadium_upgrade_err_not_upgradable"));
		}
		...
		if ($team["team_budget"] <= $costs) {
			throw new Exception(getMessage("stadium_extend_err_too_expensive"));
		}
		...
		$this->_db->queryUpdate(array(
				...
				),"stadion", "id = %d", $stadium["stadium_id"]);

		// success message
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("stadium_upgrade_success"),
				getMessage("stadium_upgrade_success_details")));

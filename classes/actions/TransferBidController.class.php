...
class TransferBidController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($clubId < 1) {
			throw new Exception(getMessage('error_action_required_team'));
		}
		...
		if ($user->id == $player['team_user_id']) {
			throw new Exception(getMessage('transfer_bid_on_own_player'));
		}

		// check if player is still on transfer list
		if (!$player['player_transfermarket']) {
			throw new Exception(getMessage('transfer_bid_player_not_on_list'));
		}
		...
		if ($parameters['contract_salary'] < $minSalary) {
			throw new Exception(getMessage('transfer_bid_salary_too_less'));
		}

		// check goal bonus
		$minGoalBonus = $player['player_contract_goalbonus'] * 1.1;
		if ($parameters['contract_goal_bonus'] < $minGoalBonus) {
			throw new Exception(getMessage('transfer_bid_goalbonus_too_less'));
		}
		...
		if ($player['team_id'] > 0) {
			...
			if ($parameters['amount'] <= $minBid) {
				throw new Exception(getMessage('transfer_bid_amount_must_be_higher', $minBid));
			}
			...
			if ($ownBidValue <= $opponentSalary) {
				throw new Exception(getMessage('transfer_bid_contract_conditions_too_low'));
			}
		...
		if (isset($bids['bidsamount']) && ($parameters['handmoney'] + $parameters['amount'] + $bids['bidsamount']) >= $team['team_budget']) {
			throw new Exception(getMessage('transfer_bid_budget_for_all_bids_too_less'));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage('transfer_bid_success'),''));

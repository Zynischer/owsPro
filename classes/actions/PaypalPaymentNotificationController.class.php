...
class PaypalPaymentNotificationController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		$header .= "Host: ". getConfig("paypal_host") . "\r\n";

		$header .= "Content-Type: application/x-www-form-urlencoded\r\n";
		$header .= "Content-Length: " . strlen($req) . "\r\n\r\n";

		$fp = fsockopen (getConfig("paypal_url"), 443, $errno, $errstr, 30);
		if (!$fp) {
			...
			while (!feof($fp)) {
				...
				if (strcmp ($res, "VERIFIED") == 0) {
					...
					if (strtolower($parameters["receiver_email"]) != strtolower(getConfig("paypal_receiver_email"))) {
						EmailHelper::sendSystemEmail($this->_websoccer,getConfig("systememail"), "Failed PayPal confirmation: Invalid Receiver",
					...
					if ($parameters["payment_status"] != "Completed") {
						EmailHelper::sendSystemEmail($this->_websoccer,getConfig("systememail"), "Failed PayPal confirmation: Invalid Status",

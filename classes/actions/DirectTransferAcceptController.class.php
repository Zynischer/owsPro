...
class DirectTransferAcceptController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($this->_websoccer->getConfig("transferoffers_adminapproval_required")) {
			...
			$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
					getMessage("transferoffer_accepted_title"),
					getMessage("transferoffer_accepted_message_approvalpending")));
			...
			$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
					getMessage("transferoffer_accepted_title"),
					getMessage("transferoffer_accepted_message")));
		}
	...
	private function checkExchangePlayer($playerId, $userId, $teamBudget) {
		...
		if ($player["player_transfermarket"]) {
			throw new Exception(getMessage("transferoffer_accept_err_exchangeplayer_on_transfermarket", $playerName));
		}

		// Check if player is still playing for the user (might have changed)
		if ($player["team_user_id"] != $userId) {
			throw new Exception(getMessage("transferoffer_accept_err_exchangeplayer_notinteam", $playerName));
		}

		// check if user can afford player
		$minBudget = 40 * $player["player_contract_salary"];
		if ($teamBudget < $minBudget) {
			throw new Exception(getMessage("transferoffer_accept_err_exchangeplayer_salarytoohigh", $playerName));
		}
	}

...
class ExecuteTrainingController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($unit["date_executed"]) {
			throw new Exception(getMessage("training_execute_err_already_executed"));
		}
		...
		foreach ($campBookings as $booking) {
			if ($booking["date_start"] <= $now && $booking["date_end"] >= $now) {
				throw new Exception(getMessage("training_execute_err_team_in_training_camp"));
			}
		}
		...
		if (isset($liveMatch["match_id"])) {
			throw new Exception(getMessage("training_execute_err_match_simulating"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("training_execute_success"),""));

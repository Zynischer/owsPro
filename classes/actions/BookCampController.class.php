...
class BookCampController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($startDateTimestamp <= $now) {
			throw new Exception(getMessage("trainingcamp_booking_err_dateinpast"));
		}
		...
		if (count($existingBookings)) {
			throw new Exception(getMessage("trainingcamp_booking_err_existingbookings"));
		}
		...
		if ($totalCosts >= $team["team_budget"]) {
			throw new Exception(getMessage("trainingcamp_booking_err_tooexpensive"));
		}
		...
		if (count($matches)) {
			throw new Exception(getMessage("trainingcamp_booking_err_matcheswithintimeframe"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("trainingcamp_booking_success"),""));

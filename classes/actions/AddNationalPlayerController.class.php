...
class AddNationalPlayerController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if ($players && $players["hits"]) {
			throw new Exception(getMessage("nationalteams_addplayer_err_alreadyinteam"));
		}
		...
		if ($existingplayers["hits"] >= 30) {
			throw new Exception(getMessage("nationalteams_addplayer_err_toomanyplayer", 30));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("nationalteams_addplayer_success"),""));

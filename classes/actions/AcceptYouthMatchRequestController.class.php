...
class AcceptYouthMatchRequestController implements IActionController {
	...
	public function executeAction($parameters) {
		...
		if (!$request) {
			throw new Exception(getMessage("youthteam_matchrequest_cancel_err_notfound"));
		}

		// check if own request
		if ($clubId == $request["team_id"]) {
			throw new Exception(getMessage("youthteam_matchrequest_accept_err_ownrequest"));
		}
		...
		$this->_websoccer->addFrontMessage(new FrontMessage(MESSAGE_TYPE_SUCCESS,
				getMessage("youthteam_matchrequest_accept_success"),
				getMessage("youthteam_matchrequest_accept_success_details")));

...
class DataUpdateSimulatorObserver implements ISimulatorObserver {
	...
	public function onBeforeMatchStarts(SimulationMatch $match) {
		...
		if ((getConfig('sim_income_trough_friendly') || $match->type !== 'Freundschaft')
	...
	private function updatePlayerOfFriendlyMatch(SimulationPlayer $player) {
		...
		if (getConfig('sim_tiredness_through_friendly')) {
			$columns['w_frische'] = $player->strengthFreshness;

			$minMinutes = (int)getConfig('sim_played_min_minutes');
			$staminaChange = (int)getConfig('sim_strengthchange_stamina');
		...
		// injury
		if ($player->injured > 0 && getConfig('sim_injured_after_friendly')) {
	...
	private function updatePlayer(SimulationMatch $match, SimulationPlayer $player, $isTeamWinner, $isTie) {
		...
		$minMinutes = (int)getConfig('sim_played_min_minutes');
		$blockYellowCards = (int)getConfig('sim_block_player_after_yellowcards');
		$staminaChange = (int)getConfig('sim_strengthchange_stamina');
		...
		// update other fields
		if (!$player->team->isNationalTeam || getConfig('sim_playerupdate_through_nationalteam')) {
			$columns['w_frische'] = $player->strengthFreshness;
			$columns['verletzt'] = $player->injured;
	...
	private function updatePlayerWhoDidNotPlay(SimulationMatch $match, $isNationalTeam, $playerinfo) {
		...
		if (!$isNationalTeam || getConfig('sim_playerupdate_through_nationalteam')) {
			$columns['w_zufriedenheit'] = max(1, $playerinfo['w_zufriedenheit'] - $satisfactionChange);
			$columns['w_frische'] = min(100, $playerinfo['w_frische'] + getConfig('sim_strengthchange_freshness_notplayed'));
		}
	...
	private function updateUsers(SimulationMatch $match) {
		$highscoreWin = getConfig('highscore_win');
		$highscoreLoss = getConfig('highscore_loss');

...
class DefaultSimulationStrategy implements ISimulationStrategy {
	...
	public function nextAction(SimulationMatch $match) {
		...
		$pAction['shoot'] = round(max($minShootProb, min($pShoot + $tacticInfluence + $resultInfluence, 50)) * getConfig('sim_shootprobability') / 100);
	...
	public function tackle(SimulationMatch $match) {
		...
		if ($result == TRUE) {
			...
			$pTackle['yellow'] = round(max(1, min(20, round((100 - $opponent->strengthTech) / 2))) * getConfig('sim_cardsprobability') / 100);
			...
			if ($tackled == 'yellow' || $tackled == 'red') {
				...
				$pInjured[TRUE] = min(99, round(((100 - $player->strengthFreshness) / 3) * getConfig('sim_injuredprobability') / 100));
				$pInjured[FALSE] = 100 - $pInjured[TRUE];
				$injured = SimulationHelper::selectItemFromProbabilities($pInjured);
				$blockedMatches = 0;
				if ($injured) {
					$maxMatchesInjured = (int)getConfig('sim_maxmatches_injured');
					$pInjuredMatches[1] = 5;
				...
				foreach ($this->_observers as $observer) {
					if ($tackled == 'yellow') {
						...
						$maxMatchesBlocked = (int)getConfig('sim_maxmatches_blocked');
				...
				if ($player->position == PLAYER_POSITION_STRIKER) {
					...
					$goalyInfluence = (int)getConfig('sim_goaly_influence');
	...
	private function _setShootStrengthPerPosition() {
		...
		$this->_shootStrengthPerPosition[PLAYER_POSITION_DEFENCE] = getConfig('sim_shootstrength_defense');
		$this->_shootStrengthPerPosition[PLAYER_POSITION_MIDFIELD] = getConfig('sim_shootstrength_midfield');
		$this->_shootStrengthPerPosition[PLAYER_POSITION_STRIKER] = getConfig('sim_shootstrength_striker');
	}

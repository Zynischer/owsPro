...
class Simulator {
    ...
    public function __construct(DbConnection $db, WebSoccer $websoccer) {
    	...
    	$strategyClass = getConfig('sim_strategy');
    ...
    private function completeMatch($match) {
    	...
    	// trigger plug-ins
    	$event = new MatchCompletedEvent($this->_websoccer, $this->_db, I18n::getInstance(getConfig('supported_languages')),
    			$match);
    ...
    private function _increaseMinutesPlayed(SimulationTeam $team) {
    	foreach ($team->positionsAndPlayers as $position => $players) {
    		foreach ($players as $player) {
    			$player->setMinutesPlayed($player->getMinutesPlayed() + 1,getConfig('sim_decrease_freshness'));
    		}
    	}
    }

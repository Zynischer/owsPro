...
class SimulationPlayer {
    ...
    public function recomputeTotalStrength(WebSoccer $websoccer, SimulationMatch $match) {
    	...
    	if ($match->isSoldOut && $this->team->id == $match->homeTeam->id) {
	    	$mainStrength += getConfig("sim_home_field_advantage");
    	}

    	// weakening of NAs
    	if ($this->team->noFormationSet) {
    		$mainStrength = round($mainStrength * getConfig("sim_createformation_strength") / 100);
    	}

    	$weightsSum = getConfig("sim_weight_strength")
    		+ getConfig("sim_weight_strengthTech")
    		+ getConfig("sim_weight_strengthStamina")
    		+ getConfig("sim_weight_strengthFreshness")
    		+ getConfig("sim_weight_strengthSatisfaction");

    	// get weights from settings
    	$totalStrength = $mainStrength * getConfig("sim_weight_strength");
    	$totalStrength += $this->strengthTech * getConfig("sim_weight_strengthTech");
    	$totalStrength += $this->strengthStamina * getConfig("sim_weight_strengthStamina");
    	$totalStrength += $this->strengthFreshness * getConfig("sim_weight_strengthFreshness");
    	$totalStrength += $this->strengthSatisfaction * getConfig("sim_weight_strengthSatisfaction");

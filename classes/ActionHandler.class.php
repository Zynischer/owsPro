...
class ActionHandler {
	...
	public static function handleAction(WebSoccer $website, DbConnection $db, I18n $i18n, $actionId) {
		...
		if (strpos($actionConfig['role'], 'admin') !== false) {
			if (!$user->isAdmin()) {
				throw new AccessDeniedException(getMessage('error_access_denied'));
			}
		} else {
			// all other actions
	...
	private static function _validateParameters($params, $website, $i18n) {
		...
		foreach ($params as $param) {
			...
			if ($required && $paramValue == null) {
				$errorMessages[$paramId] = getMessage('validation_error_required');
			} else if ($paramValue != null) {

				// minimum / maximum length
				if ($type == 'text' && $min > 0 && strlen($paramValue) < $min) {
					$errorMessages[$paramId] = sprintf(getMessage('validation_error_min_length'), $min);
				} else if ($type == 'text' && $max > 0 && strlen($paramValue) > $max) {
					$errorMessages[$paramId] = sprintf(getMessage('validation_error_max_length'), $max);

				// check number
				} else if ($type == 'number' && !is_numeric($paramValue)) {
					$errorMessages[$paramId] = getMessage('validation_error_not_a_number');
				} else if ($type == 'number' && $paramValue < $min) {
					$errorMessages[$paramId] = getMessage('validation_error_min_number', $min);
				} else if ($type == 'number' && $max > 0 && $paramValue > $max) {
					$errorMessages[$paramId] = getMessage('validation_error_max_number', $max);

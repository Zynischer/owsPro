...
foreach ($adminpage as $pageId => $pageData) {
	...
	if (isset($pageInfo['entity']) && $pageInfo['entity']) {
		$siteInfo['label'] = getMessage('entity_' . $pageInfo['entity']);
...
<html lang="<?php echo $i18n->getCurrentLanguage(); ?>">
  <head>
    <title><?php echo getMessage("main_title")?></title>
    ...
    <div class="navbar navbar-inverse navbar-fixed-top">
      <div class="navbar-inner">
        <div class="container-fluid">
          ...
          <a class="brand" href="index.php" title="<?php echo getMessage("admincenter_homelink_tooltip"); ?>"><?php echo getMessage("admincenter_brand") ?></a>
          <div class="nav-collapse collapse">
            <p class="navbar-text pull-right">
              <?php echo getMessage("admincenter_loggedin_as"); ?> <a href="?site=profile" class="navbar-link" title="<?php echo getMessage("admincenter_editprofile_tooltip"); ?>"><?php echo escapeOutput($admin['name']); ?></a> (<a href="logout.php" class="navbar-link"><?php echo getMessage("admincenter_logout"); ?></a>)
            </p>
            <ul class="nav">
              <li><a href="<?php
			  ...
			  <li><a href="logout.php"><i class="icon-off icon-white"></i> <?php echo getMessage("admincenter_logout"); ?></a></li>
            </ul>
          </div><!--/.nav-collapse -->
        </div>
      </div>
...
if (preg_match('#^[a-z0-9_-]+$#i', $site) && file_exists($includeFile) ) {
	try {
		...
		echo createErrorMessage(getMessage('alert_error_title'), $e->getMessage());
	}
} else {
	echo createErrorMessage(getMessage('alert_error_title'),getMessage('error_page_not_found'));
}
?>
        </div><!--/span-->
      </div><!--/row-->
	...
	$(function() {
		$(document).on("click", ".deleteBtn", function(e) {
			bootbox.confirm("<?php echo getMessage("manage_delete_multiselect_confirm"); ?>",
					"<?php echo getMessage("option_no"); ?>",
					"<?php echo getMessage("option_yes"); ?>",
			function(result) {
				if (result) {
		...
		$(document).on("click", ".deleteLink", function(e) {
			...
			bootbox.confirm("<?php echo getMessage("manage_delete_link_confirm"); ?>",
					"<?php echo getMessage("option_no"); ?>",
					"<?php echo getMessage("option_yes"); ?>",
			function(result) {
				if (result) {

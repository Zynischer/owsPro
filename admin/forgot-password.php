...
if ($inputEmail) {
	...
	if (count($errors) == 0) {
		...
		if($result->num_rows < 1) {
			$errors['inputEmail'] = getMessage('sendpassword_admin_usernotfound');
		} elseif ($admin['passwort_neu_angefordert'] > ($now-120*60)) {
			$errors['inputEmail'] = getMessage('sendpassword_admin_alreadysent');
		} else {
...
function _sendEmail($email, $password, $website, $i18n) {
	...
	EmailHelper::sendSystemEmailFromTemplate($website, $i18n,
		$email,
		getMessage('sendpassword_admin_email_subject'),
		'sendpassword_admin',
		$tplparameters);
}
?>
...
<html>
  <head>
    <title>AdminCenter - <?php echo getMessage('sendpassword_admin_title'); ?></title>
  ...
  <body>
	...
	<div class='container'>
		...
		<h1><?php echo getMessage('sendpassword_admin_title'); ?></h1>

<?php
if (count($errors) > 0) {
	foreach($errors as $key => $message) {
		echo createErrorMessage(getMessage('subpage_error_title'), $message);
	}

}
?>
		<p><?php echo getMessage('sendpassword_admin_intro'); ?></p>
		<form action='forgot-password.php' method='post' class='form-horizontal'>
		  <div class='control-group<?php if (isset($errors['inputEmail'])) echo ' error'; ?>'>
			<label class='control-label' for='inputEmail'><?php echo getMessage('sendpassword_admin_label_email'); ?></label>
			<div class='controls'>
		  ...
		  <div class='control-group'>
			<div class='controls'>
			  <button type='submit' class='btn'><?php echo getMessage('sendpassword_admin_button'); ?></button>
			</div>
		  </div>
		</form>

		<p><a href='login.php'><?php echo getMessage('sendpassword_admin_loginlink'); ?></a>

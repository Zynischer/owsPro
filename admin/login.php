...
if ($inputUser or $inputPassword) {
	if (!$inputUser) {
		$errors['inputUser'] = getMessage('login_error_nousername');
	}
	if (!$inputPassword) {
		$errors['inputPassword'] = getMessage('login_error_nopassword');
	}
	...
	if (count($errors) == 0) {
		...
		if($result->num_rows < 1) {
			$errors['inputUser'] = getMessage('login_error_unknownusername');
		} else {
			...
			if ($admin['passwort'] == $hashedPw || $admin['passwort_neu'] == $hashedPw) {
				...
				$errors['inputPassword'] = getMessage('login_error_invalidpassword');
				sleep(5);
			}
...
<html lang='de'>
  <head>
    <title><?php echo getMessage('login_title');?></title>
  ...
  <body>
	...
	<div class='container'>
		...
		<h1><?php echo getMessage('login_title');?></h1>

<?php
if ($forwarded) {
	echo createWarningMessage(getMessage('login_alert_accessdenied_title'),getMessage('login_alert_accessdenied_content'));
} else if ($loggedout) {
	echo createSuccessMessage(getMessage('login_alert_logoutsuccess_title'),getMessage('login_alert_logoutsuccess_content'));
} else if ($newpwd) {
	echo createSuccessMessage(getMessage('login_alert_sentpassword_title'),getMessage('login_alert_sentpassword_content'));
} else if (count($errors) > 0) {
	echo createErrorMessage(getMessage('login_alert_error_title'),getMessage('login_alert_error_content'));
}
?>
		...
		<form action='login.php' method='post' class='form-horizontal'>
		  <div class='control-group<?php if (isset($errors['inputUser'])) echo ' error'; ?>'>
			<label class='control-label' for='inputUser'><?php echo getMessage('login_label_user');?></label>
			<div class='controls'>
			  <input type='text' name='inputUser' id='inputUser' placeholder='<?php echo getMessage('login_label_user');?>' required>
			</div>
		  </div>
		  <div class='control-group<?php if (isset($errors['inputPassword'])) echo ' error'; ?>'>
			<label class='control-label' for='inputPassword'><?php echo getMessage('login_label_password');?></label>
			<div class='controls'>
			  <input type='password' name='inputPassword' id='inputPassword' placeholder='<?php echo getMessage('login_label_password');?>' required>
			</div>
		  </div>
		  <div class='control-group'>
			<div class='controls'>
			  <button type='submit' class='btn'><?php echo getMessage('login_button_logon');?></button>
			</div>
		  </div>
		</form>

		<p><a href='forgot-password.php'><?php echo getMessage('login_link_forgotpassword');?></a>

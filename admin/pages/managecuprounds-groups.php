...
$mainTitle = getMessage("managecuprounds_groups_navlabel");

echo "<h1>$mainTitle</h1>";

if (!$admin["r_admin"] && !$admin["r_demo"] && !$admin["r_spiele"]) {
	throw new Exception(getMessage("error_access_denied"));
}
...
$result = $db->querySelect("R.id AS round_id,R.name AS round_name,firstround_date,secondround_date,C.id AS cup_id,C.name as cup_name",
		"_cup_round AS R INNER JOIN cup AS C ON C.id = R.cup_id",
		"R.id = %d", $roundid);
...
echo "<h2>". getMessage("entity_cup") . " - " . escapeOutput($round["round_name"]) . "</h2>";

echo "<p><a href=\"?site=managecuprounds&cup=". $round["cup_id"] . "\" class=\"btn\">" . getMessage("managecuprounds_groups_back") ."</a></p>";
...
$result = $db->querySelect("T.id AS team_id,T.name AS team_name,L.name AS league_name,L.land AS league_country",
		"_verein AS T INNER JOIN liga AS L ON L.id = T.liga_id",
...
if ($action == "create") {
	...
	if (isset($_REQUEST["groupname"]) && strlen(trim($_REQUEST["groupname"]))) {
		...
		$db->queryUpdate($columns,"cup_round_group", "cup_round_id = %d AND name = '%s'",
				array($roundid, $_REQUEST["group"]));

		$db->queryUpdate(array("groupname" => $_REQUEST["groupname"]),"cup_round_group_next", "cup_round_id = %d AND groupname = '%s'",
				array($roundid, $_REQUEST["group"]));

		$db->queryUpdate(array("pokalgruppe" => $_REQUEST["groupname"]),"spiel", "pokalname = '%s' AND pokalrunde = '%s' AND pokalgruppe = '%s'",
	...
	$db->queryInsert($columns,"cup_round_group");
	...
	$result = $db->querySelect("COUNT(*) AS teams","cup_round_group", "cup_round_id = %d AND name = '%s'", array($roundid, $groupName));
...
$fromTable = "verein AS C ON C.id = G.team_id";
...
if (count($groups)) {
	...
	$result = $db->querySelect("*","cup_round", "cup_id = %d AND id != %d ORDER BY firstround_date ASC", array($round["cup_id"], $round["round_id"]));
	$rounds = array();
	...
	$result = $db->querySelect("*","cup_round_group_next", "cup_round_id = %d", array($roundid));
	$rankConfigs = array();
	...
	<table class="table table-bordered">
		...
		<thead>
			<tr>
				<th><?php echo getMessage("managecuprounds_group_label_name"); ?></th>
				<th><?php echo getMessage("managecuprounds_group_label_teams"); ?></th>
				<th><?php echo getMessage("managecuprounds_groups_nextrounds"); ?></th>
				<th>&nbsp;</th>
				<th>&nbsp;</th>
			</tr>
		</thead>
		...
		foreach($groups as $groupName => $groupItems) {
			...
			if ($showEditForm && $_REQUEST["group"] == $groupName) {
				...
				<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="form-inline">
					...
					<input type="submit" class="btn" value="<?php echo getMessage("button_save"); ?>">
				</form>
				<?php
			} else {
			...
			foreach ($groupItems as $groupItem) {
				echo "<li>" . escapeOutput($groupItem["team_name"]) . " <a class=\"deleteLink\" href=\"?site=". $site . "&round=". $roundid . "&group=". urlencode(escapeOutput($groupName)) . "&action=deletegroupassignment&teamid=". $groupItem["team_id"] . "\" title=\"". getMessage("manage_delete") . "\"><i class=\"icon-remove-sign\"></i></a></li>";
				$noOfTeams++;
			}
			echo "</ul>\n";
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="form-inline">
				...
				<input type="submit" class="btn btn-small" value="<?php echo getMessage("managecuprounds_groups_addteam"); ?>">
			</form>
			<?php
			...
			echo "</ol>";
			?>
			<input type="submit" class="btn btn-small" value="<?php echo getMessage("button_save"); ?>">
			</form>
			<?php
			echo "</td>";
			echo "<td><a href=\"?site=". $site . "&round=". $roundid . "&action=edit&group=". urlencode(escapeOutput($groupName)) . "\" title=\"". getMessage("manage_edit") . "\"><i class=\"icon-pencil\"></i></a></td>";
			echo "<td><a class=\"deleteLink\" href=\"?site=". $site . "&round=". $roundid . "&group=". urlencode(escapeOutput($groupName)) . "&action=delete\" title=\"". getMessage("manage_delete") . "\"><i class=\"icon-trash\"></i></a></td>";
	...
	<div class="well">
		...
		<p><a href="<?php echo $matchesUrl; ?>"><strong><?php echo $matches["hits"]; ?></strong> <?php echo getMessage("managecuprounds_groups_created_matches"); ?></a></p>
		<?php } ?>
		<p><a href="#generateModal" role="button" class="btn" data-toggle="modal"><?php echo getMessage("managecuprounds_groups_open_generate_matches_popup"); ?></a></p>
	</div>
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
		...
		<div id="generateModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="generateModalLabel" aria-hidden="true">
		  <div class="modal-header">
		    ...
		    <h3 id="generateModalLabel"><?php echo getMessage("managecuprounds_groups_generate_title"); ?></h3>
		  </div>
		  <div class="modal-body">
		  	<div class="alert">
		  		<?php echo getMessage("managecuprounds_groups_generate_alert"); ?>
		  	</div>
			<?php
		  ...
		  <div class="modal-footer">
		    <button class="btn btn-primary" type="submit"><?php echo getMessage("managecuprounds_groups_generate_submit"); ?></button>
		    <button class="btn" data-dismiss="modal" aria-hidden="true"><?php echo getMessage("button_cancel"); ?></button>
		  </div>
		</div>
	</form>
...
?>
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
	...
	<fieldset>
    <legend><?php echo getMessage("managecuprounds_groups_label_create"); ?></legend>
	...
	?>
	<div>
	<label><?php echo getMessage("managecuprounds_groups_label_selectteams"); ?></label>
	</div>
		...
		<div style="width: 600px; height: 300px; overflow: auto; border: 1px solid #cccccc;">
			<table class="table table-striped table-hover">
				...
				<thead>
					<tr>
						<th>&nbsp;</th>
						<th><?php echo getMessage("entity_club")?></th>
						<th><?php echo getMessage("entity_league")?></th>
					</tr>
				</thead>
				<tbody>
					<?php
	...
	<?php
	echo "<a href=\"?site=managecuprounds&cup=". $round["cup_id"] . "\" class=\"btn\">" . getMessage("button_cancel") ."</a>";

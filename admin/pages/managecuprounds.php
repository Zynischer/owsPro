...
//##### Infos zur Rubrik #####
$mainTitle = getMessage("managecuprounds_navlabel");
$r_prefix = ""; #Prefix der Datei

echo "<h1>$mainTitle</h1>";

echo "<p><a href=\"?site=manage&entity=cup\" class=\"btn\">" . getMessage("button_cancel") ."</a></p>";
...
<div class="alert alert-info">
	<h5><?php echo getMessage("managecuprounds_infoalert_title"); ?></h5>
	<p><?php echo getMessage("managecuprounds_infoalert_msg"); ?></p>
</div>

<?php

echo "<h2>". getMessage("entity_cup") . ": " . escapeOutput($cup["name"]) . "</h2>";
...
function renderRound($roundNode) {
	...
	if($showEditForm) {
		...
		<div class="control-group">
			<div class="controls">
				...
				<a href="<?php echo "?site=" . $site . "&cup=" . $cupid; ?>" class="btn"><?php echo getMessage("button_cancel"); ?></a>
			</div>
		</div>
	  </form>
	...
	// display details
		...
		echo " <a href=\"?site=". $site . "&cup=". $cupid . "&action=edit&id=". $roundNode["round"]["id"] . "\" title=\"". getMessage("manage_edit") . "\"><i class=\"icon-pencil\"></i></a>";
		echo " <a class=\"deleteLink\" href=\"?site=". $site . "&cup=". $cupid . "&action=delete&id=". $roundNode["round"]["id"] . "\" title=\"". getMessage("manage_delete") . "\"><i class=\"icon-trash\"></i></a>";
		...
		echo "<li><a href=\"$matchesUrl\">". getMessage("managecuprounds_show_matches") . "</a></li>";
		...
		if (!$roundNode["round"]["groupmatches"]) {
			echo "<p><a href=\"$addMatchUrl\" class=\"btn btn-mini\"><i class=\"icon-plus-sign\"></i> ". getMessage("managecuprounds_add_match") . "</a>";
			echo " <a href=\"?site=managecuprounds-generate&round=". $roundNode["round"]["id"] . "\" class=\"btn btn-mini\"><i class=\"icon-random\"></i> ". getMessage("managecuprounds_generate_matches") . "</a>";
			echo "</p>";
		} else {
			echo "<p><a href=\"?site=managecuprounds-groups&round=". $roundNode["round"]["id"] . "\" class=\"btn btn-mini\"><i class=\"icon-list\"></i> ". getMessage("managecuprounds_manage_groups") . "</a>";
			echo "</p>";
		}

		if (isset($roundNode["winnerround"])) {
				echo "<p><em>". getMessage("managecuprounds_next_round_winners") . ":</em></p>\n";
				renderRound($hierarchy[$roundNode["winnerround"]]);
		}

		if (isset($roundNode["looserround"])) {
				echo "<p><em>". getMessage("managecuprounds_next_round_loosers") . ":</em></p>\n";
...
?>
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
	...
	<fieldset>
    <legend><?php echo getMessage("managecuprounds_label_create"); ?></legend>
	...
	<div class="control-group">
		<label class="control-label" for="from_round_id"><?php echo getMessage("managecuprounds_label_previous_round")?></label>

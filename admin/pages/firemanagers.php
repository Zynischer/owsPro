...
$mainTitle = getMessage('firemanagers_navlabel');
...
if (!$show) {
  ...
  <p><?php echo getMessage('firemanagers_introduction'); ?></p>

  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
	<input type="hidden" name="site" value="<?php echo $site; ?>">

	<fieldset>
    <legend><?php echo getMessage('firemanagers_search_label'); ?></legend>
	...
	<div class="form-actions">
		<input type="submit" class="btn btn-primary" accesskey="s" title="Alt + s" value="<?php echo getMessage('button_search'); ?>">
		<input type="reset" class="btn" value="<?php echo getMessage('button_reset'); ?>">
	</div>
  </form>
  ...
  <?php
	...
	if (!empty($_POST['maxbudget']) || !empty($_POST['lastlogindays']) || !empty($_POST['maxplayers']) || !empty($_POST['userid'])) {
		...
		$columns = array(
			...
			'(SELECT COUNT(*) FROM spieler AS PlayerTab WHERE PlayerTab.verein_id = C.id AND status = \'1\')' => 'team_players',
			'U.id' => 'user_id',
			'U.nick' => 'user_nick',
			'U.lastonline' => 'user_lastonline'
		);

		$fromTable = 'verein AS C';
		...
		if (!empty($_POST['maxplayers'])) {
			$whereCondition .= ' OR (SELECT COUNT(*) FROM spieler AS PlayerTab WHERE PlayerTab.verein_id = C.id AND status = \'1\') < %d';
		...
		if (!$result->num_rows) {
			echo createInfoMessage(getMessage('firemanagers_search_nohits'), '');
		} else {
			...
			<form action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post' class='form-horizontal' id='frmMain' name='frmMain'>
				...
				<table class='table table-striped table-hover'>
					<thead>
						<tr>
							...
							<th><?php echo getMessage('entity_users'); ?></th>
							<th><?php echo getMessage('entity_users_lastonline'); ?></th>
							<th><?php echo getMessage('entity_club_finanz_budget'); ?></th>
							<th><?php echo getMessage('entity_player'); ?></th>
						</tr>
					</thead>
					<tbody>
					<?php
				...
				<p><input type='submit' class='btn btn-primary' accesskey='l' title='Alt + l' value='<?php echo getMessage('firemanagers_dismiss_button'); ?>'></p>
			</form>
...
elseif ($show == 'selectoptions') {
	if (!isset($_POST['selectedteams']) || !count($_POST['selectedteams'])) {
		echo createErrorMessage(getMessage('firemanagers_dismiss_noneselected'), '');
		echo '<a href=\'?site='. $site .'\' class=\'btn\'>'. getMessage('back_label') .'</a>';
	} else {
?>
  ...
  <form action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post' class='form-horizontal'>
	...
	<fieldset>
    <legend><?php echo getMessage('firemanagers_dismiss_label'); ?></legend>
	...
	<fieldset>
    <legend><?php echo getMessage('firemanagers_dismiss_label_players'); ?></legend>
	...
	<fieldset>
    <legend><?php echo getMessage('firemanagers_dismiss_label_minteamsize'); ?></legend>
    <p><?php echo getMessage('firemanagers_dismiss_label_minteamsize_intro'); ?></p>
	<?php
	$formFields = array();
	...
	<div class='form-actions'>
		<input type='submit' class='btn btn-primary' accesskey='s' title='Alt + s' value='<?php echo getMessage('firemanagers_dismiss_button'); ?>'>
		<a class='btn' href='?site=<?php echo $site; ?>'><?php echo getMessage('button_cancel'); ?></a>
	</div>
  </form>
...
elseif ($show == 'dismiss') {
  ...
  if (isset($err)) {
	...
	foreach($teamIds as $teamId) {
		...
		$db->queryUpdate($teamcolumns,'verein', 'id = %d', $teamId);

		// disable user
		if (isset($_POST['disableusers']) && $_POST['disableusers']) {
			$db->queryUpdate(array('status' => '0'),'user', 'id = %d', $team['user_id']);
		}
		...
		$result = $db->querySelect('id, position_main, w_kondition, w_frische, w_zufriedenheit, vertrag_spiele','spieler',
	...
	echo createSuccessMessage(getMessage('firemanagers_dismiss_success'), '');

    echo '<p>&raquo; <a href=\'?site='. $site .'\'>'. getMessage('back_label') . '</a></p>';

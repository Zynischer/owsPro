...
$mainTitle = getMessage('entitylogging_navlabel');
...
if (!$show) {
  ...
  <p><?php echo getMessage('entitylogging_intro'); ?></p>
    ...
    if (!$datei_gr) echo '<p>'. getMessage('empty_list') . '</p>';
      ...
      ?>
            ...
            <table class='table table-bordered table-striped' style='margin-top: 10px'>
              <tr>
                <th><?php echo getMessage('entitylogging_label_no'); ?></th>
                <th><?php echo getMessage('entitylogging_label_time'); ?></th>
                <th><?php echo getMessage('entitylogging_label_user'); ?></th>
                <th><?php echo getMessage('entitylogging_label_type'); ?></th>
                <th><?php echo getMessage('entitylogging_label_data'); ?></th>
              </tr>
              <?php
                ...
                echo '<tr>
                  ...
                  <td>';
                  	...
                  	if ($row[3] == LOG_TYPE_EDIT) {
						echo '<span class=\'label label-info\'><i class=\'icon-white icon-pencil\'></i> '. getMessage('entitylogging_action_edit') . '</span>';
					} elseif ($row[3] == LOG_TYPE_DELETE) {
						echo '<span class=\'label label-important\'><i class=\'icon-white icon-trash\'></i> '. getMessage('entitylogging_action_delete') . '</span>';
					} else {
						echo $row[3];
					}
                  echo '</td>
				  <td>'. getMessage('entity_' . $row[4]) .': { ';

...
$mainTitle = getMessage("playersgenerator_navlabel");
...
if (!$show) {
  ...
  ?>
    ...
    <form class="form-inline">
  		<label for="leagueid"><?php echo getMessage("generator_label_league") ?></label>
	  	...
	  	<button type="submit" class="btn btn-primary"><?php echo getMessage("button_display") ?></button>
	  	<a href="index.php?site=<?php echo $site ?>" class="btn"><?php echo getMessage("button_reset") ?></a>

	  	<input type="hidden" name="site" value="<?php echo $site ?>" />
	</form>

	<p><a href="index.php?site=<?php echo $site ?>&show=generateform&transfermarket=1" class="btn"><?php echo getMessage("playersgenerator_generator_for_transfermarket") ?></a></p>
  ...
  if ($leagueid > 0) {
	  ...
	  $columns["(SELECT COUNT(*) FROM spieler AS S WHERE S.verein_id = T1.id)"] = "playerscount";

	  $fromTable = "verein AS T1";
	  $whereCondition = "T1.liga_id = %d ORDER BY T1.name ASC";
	  $result = $db->querySelect($columns, $fromTable, $whereCondition, $leagueid);

	  if (!$result->num_rows) {
	  	echo "<p>" . getMessage("playersgenerator_noteams") . "</p>";
	  } else {
	  	?>

	  	<p><a href="?site=<?php echo $site ?>&show=generateform&leagueid=<?php echo $leagueid ?>"
	  		class="btn"><?php echo getMessage("playersgenerator_create_for_all_teams"); ?></a></p>

	  	<h4 style="margin-top:20px"><?php echo getMessage("playersgenerator_create_for_single_teams"); ?></h4>

	    <table class="table table-striped">
	    	<thead>
	    		<tr>
	    			<th><?php echo getMessage("entity_club_name"); ?></th>
	    			<th><?php echo getMessage("playersgenerator_head_playerscount"); ?></th>
	    		</tr>
	    	</thead>
	    	<tbody>
	    	<?php
...
elseif ($show == "generateform") {
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
	...
	<fieldset>
    <legend><?php echo getMessage("generator_label"); ?></legend>
	...
	<div class="form-actions">
		<input type="submit" class="btn btn-primary" accesskey="s" title="Alt + s" value="<?php echo getMessage("generator_button"); ?>">
...
$_POST['entity_player_vertrag_gehalt'], $_POST['entity_player_vertrag_spiele'], $strengths, $positions, $_POST["playersgenerator_label_deviation"], $_POST['entity_player_nation']);
	...
	echo createSuccessMessage(getMessage("generator_success"), "");

      echo "<p>&raquo; <a href=\"?site=". $site ."&leagueid=". $leagueid . "\">". getMessage("back_label") . "</a></p>\n";

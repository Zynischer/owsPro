...
$mainTitle = getMessage("schedulegenerator_navlabel");
...
if (!$show) {
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal">
	...
	<fieldset>
    <legend><?php echo getMessage("schedulegenerator_label"); ?></legend>
...
elseif ($show == "generate") {
  ...
  if (!isset($_POST['league']) || $_POST['league'] <= 0) $err[] = getMessage("generator_validationerror_noleague");
  if (!isset($_POST['rounds']) || $_POST['rounds'] <= 0 || $_POST['rounds'] > 10) $err[] = getMessage("schedulegenerator_err_invalidrounds");
  if (!isset($_POST['timebreak']) || $_POST['timebreak'] <= 0 || $_POST['timebreak'] > 50) $err[] = getMessage("schedulegenerator_err_invalidtimebreak");
  ...
  //##### Save #####
	...
	// get teams
  	$result = $db->querySelect("id","verein", "liga_id = %d", $_POST['league']);
	if (!$result->num_rows) {
	...
	$db->queryInsert($seasoncolumns,"saison");

...
echo "<h1>".  getMessage("match_manage_complete") . "</h1>";
...
if ($match["match_simulated"]) {
	throw new Exception(getMessage("match_manage_complete_alreadysimulated"));
}
...
if ($action == "complete") {
	...
	echo createSuccessMessage(getMessage("match_manage_complete_success"), "");

}

echo "<h3><a href=\"". getInternalUrl("team", "id=" . $match["match_home_id"]) . "\" target=\"_blank\">". escapeOutput($match["match_home_name"]) . "</a> - <a href=\"". getInternalUrl("team", "id=" . $match["match_guest_id"]) . "\" target=\"_blank\">". escapeOutput($match["match_guest_name"]) . "</a></h3>";

echo "<div class=\"well\">". getMessage("match_manage_complete_intro") . "</div>";
...
echo "<button type=\"submit\" class=\"btn btn-primary\">". getMessage("match_manage_complete_button") . "</button>";
echo "</div></form>";

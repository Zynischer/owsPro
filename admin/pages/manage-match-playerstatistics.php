...
echo "<h1>".  getMessage("match_manage_playerstatistics") . "</h1>";

if (!$admin["r_admin"] && !$admin["r_demo"] && !$admin["r_spiele"]) {
	throw new Exception(getMessage("error_access_denied"));
}

echo "<p><a href=\"?site=manage&entity=match\" class=\"btn\">". getMessage("back_label") . "</a></p>";
...
elseif ($action == "generate") {
	...
	$result->free();
	if (!$formation) {
		throw new Exception(getMessage("match_manage_playerstatistics_noformationavailable"));
	}
...
echo "<fieldset><legend>".getMessage("match_manage_createplayer_title") ."</legend>";

echo "<div class=\"control-group\">";
echo "<label class=\"control-label\" for=\"team_id\">". getMessage("entity_player_verein_id") . "</label>";
...
echo "<div class=\"control-group\">";
echo "<label class=\"control-label\" for=\"position\">". getMessage("entity_player_position_main") . "</label>";
echo "<div class=\"controls\">";
echo "<select name=\"position\" id=\"position\">";
foreach ($positions as $position) {
	echo "<option value=\"". $position . "\">". getMessage("option_" . $position). "</option>";
}
echo "</select>";
echo "</div>";
echo "</div>";
...
echo "<button type=\"submit\" class=\"btn btn-primary\">". getMessage("button_save") . "</button>";
echo "</div></form>";
...
foreach ($teamPrefixes as $teamPrefix) {
	...
	if (!$playersCount) {
		...
		if ($formationCount && $formationCount["hits"]) {
			echo "<p><a href=\"?site=$site&match=$matchId&team=$teamPrefix&action=generate\" class=\"btn\"><i class=\"icon-hand-right\"></i> ". getMessage("match_manage_playerstatistics_createfromfrmation") . "</a></p>";
		} else {
			echo "<p><i class=\"icon-warning-sign\"></i> ". getMessage("match_manage_playerstatistics_noformationavailable") . "</p>";
		}
		...
		echo "<thead>";
		echo "<tr>";
		echo "<th>". getMessage("entity_player_position") . "</th>";
		echo "<th>". getMessage("entity_player") . "</th>";

		foreach ($formFields as $formField) {
			echo "<th>". getMessage("match_manage_" . $formField) . "</th>";
		}
		...
		while ($player = $result->fetch_array()) {
			...
			foreach ($positions as $position) {
				...
				echo ">". getMessage("option_" . $position). "</option>";
			}
			echo "</select><br/>";
			echo "<select name=\"" . $fieldPrefix . "_feld\" class=\"input-medium\">";
			echo "<option value=\"1\"";
			if ($player["feld"] === "1") echo " selected";
			echo ">". getMessage("match_manage_position_field_1") . "</option>";

			echo "<option value=\"Ersatzbank\"";
			if ($player["feld"] === "Ersatzbank") echo " selected";
			echo ">". getMessage("match_manage_position_field_bench") . "</option>";

			echo "<option value=\"Ausgewechselt\"";
			if ($player["feld"] === "Ausgewechselt") echo " selected";
			echo ">". getMessage("match_manage_position_field_substituted") . "</option>";
			echo "</select></td>";

			// name
			echo "<td>". $player["name"];
			echo " <a href=\"?site=$site&action=delete&match=$matchId&player=". $player["spieler_id"] . "\" title=\"". getMessage("manage_delete") . "\" class=\"deleteLink\"><i class=\"icon-trash\"></i></a>";
			echo "</td>";

			// statistics
			foreach ($formFields as $formField) {
				echo "<td><input type=\"text\" class=\"input-mini\" name=\"". $fieldPrefix . "_". $formField . "\" title=\"". getMessage("match_manage_" . $formField) . "\" value=\"". $player[$formField] . "\"/></td>";
			}
		...
		// substitutions
		echo "<h4>". getMessage("match_manage_substitutions") . "</h4>";
		?>
		<table class="table table-striped">
			<thead>
				<tr>
					<th><?php echo getMessage("match_manage_substitutions_out") ?></th>
					<th><?php echo getMessage("match_manage_substitutions_in") ?></th>
					<th><?php echo getMessage("match_manage_substitutions_condition") ?></th>
					<th><?php echo getMessage("match_manage_substitutions_minute") ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				for ($subNo = 1; $subNo <= 3; $subNo++) {
					...
					echo ">". getMessage("match_manage_substitutions_condition_tie") . "</option>";

					echo "<option value=\"Leading\"";
					if ($match["match_". $teamPrefix . "_sub" . $subNo . "_condition"] == "Leading") echo " selected";
					echo ">". getMessage("match_manage_substitutions_condition_leading") . "</option>";

					echo "<option value=\"Deficit\"";
					if ($match["match_". $teamPrefix . "_sub" . $subNo . "_condition"] == "Deficit") echo " selected";
					echo ">". getMessage("match_manage_substitutions_condition_deficit") . "</option>";
...
echo "<div class=\"form-actions\">";
echo "<button type=\"submit\" class=\"btn btn-primary\">". getMessage("button_save") . "</button>";
echo " <button type=\"reset\" class=\"btn\">". getMessage("button_reset") . "</button>";
echo "</div></form>";
